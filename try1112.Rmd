...
to minimize $\lambda A_2+(1-\lambda)A_3$
...
$$\lambda^\ast\sum_{i=1}^nx_i^2+(1-\lambda^\ast)\sum_{i=1}^nx_i^3$$

$x_i$ 代表某兩列 coincidence 數。

```{r}
weight_J <- function(x, lambda) {
  return(lambda*sum(x^2) + (1-lambda)*sum(x^3))
}

# genetic algorithm

GA <- function(m, n, lambda, max_iter=1000, pop_size=100, mutation_rate=0.1) {
  stop_flag <- FALSE
    ...
    for (iter in 1:max_iter) {
      ...
      for (i in 1:pop_size) {
        fitness[i] <- weight_J(pop[i, ], lambda)
      }
      ...
      # select survivors
      for (i in 1:pop_size) {
        if (weight_J(children[i, ], lambda) < weight_J(pop[i, ], lambda)) {
          pop[i, ] <- children[i, ]
        }
      }
...
lambda_star <- lambda_a / (lambda_a + lambda_b)

start_time <- Sys.time()
opt <- GA(K_function(design1, 1) * N * (N-1) / 2, choose(N, 2), lambda_star, max_iter = 12000)
print(Sys.time() - start_time)
table(opt)

weight_J(delta[upper.tri(delta)], lambda_star)
weight_J(opt, lambda_star)
```
